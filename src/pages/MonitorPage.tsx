...
// 煤矿宿舍区监控区域分类
const dormitoryAreas = {
  'dormitory': { name: '宿舍楼', color: '#1890ff', icon: <SafetyOutlined /> },
  'entrance': { name: '出入口', color: '#52c41a', icon: <CarOutlined /> },
  'canteen': { name: '职工食堂', color: '#faad14', icon: <ThunderboltOutlined /> },
  'parking': { name: '停车场', color: '#722ed1', icon: <EnvironmentOutlined /> },
  'recreation': { name: '活动中心', color: '#13c2c2', icon: <VideoCameraOutlined /> },
  'safety': { name: '安全通道', color: '#ff4d4f', icon: <FireOutlined /> }
};
...
const MonitorPage: React.FC = () => {
  ...
  return (
    <div style={{ padding: 24, background: '#f0f2f5', minHeight: '100vh' }}>
      ...
      {showAlerts && (
        <Alert
          message="煤矿宿舍区视频监控系统运行正常"
          description={`当前${stats.online}个摄像头正常运行，${stats.recording}个设备正在录像。宿舍楼、食堂、停车场等关键区域监控正常，为职工生活安全保驾护航。`}
          type="success"
          showIcon
          closable
        ...
        <Space>
          ...
          <Select
            ...
            options={[
              ...
              ...Object.entries(dormitoryAreas).map(([key, area]) => ({
                value: key,
                label: area.name
              }))
            ]}
      ...
      <Row gutter={[16, 16]} style={{ minHeight: '400px' }}>
        {paginatedCameras.map(cam => {
          const areaInfo = dormitoryAreas[cam.area as keyof typeof dormitoryAreas];
          return (
              ...
              <Col span={12}>
                <Descriptions bordered column={1} size="small">
                  ...
                  <Descriptions.Item label="监控区域">
                    {dormitoryAreas[enhancedCameras.find(c => c.id === selected.id)?.area as keyof typeof dormitoryAreas]?.name}
                  </Descriptions.Item>
